...
Console.WriteLine("Hello, Sns User Api! This Api' messages consume by Sqs.User.Consumer");
...
userRoute.MapPost("/", async (CreateUser user, IAmazonSnsService amazonSqsService) =>
{
  var response =
...
userRoute.MapPatch("/", async (UpdateUser user, IAmazonSnsService amazonSqsService) =>
{
  var response =
    await amazonSqsService.PublishStandartMessageAsync(user, QueueNames.UserQueueName,
      user.GetSnsContractMessageAttributes());
